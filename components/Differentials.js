          ...
          {differentials.map((item, index) => (
            <motion.div key={index} variants={itemVariants} className="group relative">
              <div className="relative h-full p-8 md:p-10 rounded-2xl overflow-hidden transition-all duration-300 transform group-hover:translate-y-[-5px]">
                {/* Fondo glassmorphism */}
                <div className="absolute inset-0 bg-white/30 dark:bg-gray-800/30 backdrop-blur-xl rounded-2xl border border-white/20 dark:border-gray-700/20 shadow-lg z-0"></div>

                {/* Gradiente de fondo */}
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${item.color} ${item.hoverColor} opacity-10 dark:opacity-20 transition-opacity duration-300 rounded-2xl z-0`}
                ></div>
                ...
                <div className="relative z-10 flex flex-col items-center text-center">
                  ...
                  <h3 className="text-xl md:text-2xl font-bold mb-4 text-gray-900 dark:text-white transition-colors duration-300">
                    {item.title}
                  </h3>
